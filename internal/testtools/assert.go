...
// AssertContains checks if the input string contains
// all the substrs specified on the substrs argument.
//
// In case it doesn't contain any of the substrings it
// will fail the test with an appropriate error message.
func AssertContains(t *testing.T, str string, substrs ...string) {
	for _, substr := range substrs {
		require.True(t,
			strings.Contains(str, substr),
			"missing substring '%s' in input text: '%s'",
			substr, str,
		)
	}
}
